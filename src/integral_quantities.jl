...
function calcCorrelationLengths(R11::Array{Float64,1}, R22::Array{Float64,1}, R33::Array{Float64,1}, x::Array{Float32,1}, y::Array{Float32,1}, z::Array{Float32,1}, grid::rectilinearGrid)
    ...
    i2 = try i0 - 1 + findfirst(x -> x < 0.0, R11[i0:end]) - 1 catch; length(R11) end
    i1 = try findlast(x -> x < 0.0, R11[1:i0]) + 1 catch; 1 end
    j2 = try j0 - 1 + findfirst(x -> x < 0.0, R22[j0:end]) - 1 catch; length(R22) end
    j1 = try findlast(x -> x < 0.0, R22[1:j0]) + 1 catch; 1 end
    k2 = try k0 - 1 + findfirst(x -> x < 0.0, R33[k0:end]) - 1 catch; length(R33) end
    k1 = try findlast(x -> x < 0.0, R33[1:k0]) + 1 catch; 1 end
    # Calculate correlation lengths
